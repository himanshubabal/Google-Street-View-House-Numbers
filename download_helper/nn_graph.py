...
class TF_Graph():
    ...
    def create_graph(self):
        with self.graph.as_default():
            ...
            Y1 = conv_pipeline(Y1, in_width=K, out_width=L, fltr_conv=3, stride_conv=1,
                               is_train=is_train, iteration=iteration, pkeep=pkeep, token=2)
            Y1 = conv_pipeline(Y1, in_width=L, out_width=M, fltr_conv=5, stride_conv=1,
            ...
            with tf.name_scope('optimisers'):
                ...
                optimizer_all = tf.train.AdamOptimizer(learning_rate=alpha,
                                                       beta1=0.9, beta2=0.999,
                                                       epsilon=1e-08,
                                                       name="optimizer_digits").minimize(loss_total)
