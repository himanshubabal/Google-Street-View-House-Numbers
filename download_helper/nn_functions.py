...
# def batchnorm(x, is_training, iteration, conv=False, offset=0.0, scale=1.0, token=1):
#     """
#     Credits
#     -------
...
#     exp_moving_avg = tf.train.ExponentialMovingAverage(0.9999, iteration, name='exp_moving_avg_' + str(token))
#     bnepsilon = 1e-5

#     # calculate mean and variance for batch of logits
#     if conv:
#         mean, variance = tf.nn.moments(x, [0, 1, 2], name='moments_' + str(token))
#     else:
#         # mean and variance along the batch
#         mean, variance = tf.nn.moments(x, [0], name='moments_' + str(token))
...
#     beta = tf.Variable(initial_value=beta_init(param_shape), name="beta_" + str(token))

#     # Scale
#     gamma_init = tf.constant_initializer(scale)
#     gamma = tf.Variable(initial_value=gamma_init(param_shape), name="gamma_" + str(token))

#     # Apply Batch Norm
#     Ybn = tf.nn.batch_normalization(x, m, v, offset=beta, scale=gamma,
#                                     variance_epsilon=bnepsilon, name='batchnorm_' + str(token))
#     return Ybn
...
#         W = tf.Variable(tf.truncated_normal([in_nodes, num_nodes], stddev=0.1), name='W3' + str(token))
#         B = tf.Variable(tf.constant(0.1, tf.float32, [num_nodes]), name='B3' + str(token))

#         Y_fc = tf.matmul(X_in, W, name='Y_fcc_' + str(token)) + B
#         # Y_bn = batchnorm(Y_fc, is_train, iteration, conv=conv, token=30 + token)
#         # Y_dp = tf.nn.dropout(Y_bn, keep_prob=pkeep)
#         Y_dp = tf.nn.dropout(Y_fc, keep_prob=pkeep)
...
# # Convolution Laytr
# def conv_pipeline(X_in, in_width, out_width, fltr_conv, stride_conv, is_train, iteration, pkeep,
#                   token=1, conv=True):
#     with tf.name_scope('convolution_' + str(token)):
#         W = tf.Variable(tf.truncated_normal([fltr_conv, fltr_conv, in_width, out_width],
#                                             stddev=0.1), name='W1' + str(token))
#         B = tf.Variable(tf.constant(0.1, tf.float32, [out_width]), name='B1' + str(token))

#         Y_conv = tf.add(tf.nn.conv2d(X_in, W, strides=[1, stride_conv, stride_conv, 1], padding='SAME',
#                                      name='Y_conv_' + str(token)), B, name='conv_' + str(token)
#                         + '_add_' + str(token))

#         # Y_bnorm = batchnorm(Y_conv, is_train, iteration, conv=conv, token=10 + token)
#         # Y_drop = tf.nn.dropout(Y_bnorm, pkeep)
#         Y_drop = tf.nn.dropout(Y_conv, pkeep)
...
def leaky_relu(x, rate=0.01):
    leak_rate = tf.multiply(x, rate)
    activation = tf.maximum(x, leak_rate)
    return activation
...
def batchnorm(x, is_training, iteration, conv=False, offset=0.0, scale=1.0):
    ...
    param_shape = mean.get_shape().as_list()
    beta_init = tf.constant_initializer(offset)
    beta = tf.Variable(initial_value=beta_init(param_shape))

    # Scale
    gamma_init = tf.constant_initializer(scale)
    gamma = tf.Variable(initial_value=gamma_init(param_shape))
...
# Calculate loss for digits
def multi_digit_loss(logits_list, Y_, max_digits=11):
    # LOSSES FOR EACH DIGIT BRANCH
    losses = [None] * (max_digits)
    for i in range(max_digits):
        losses[i] = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_list[i], labels=Y_[:, i])
...
def fc_pipeline(X_in, num_nodes, is_train, iteration, pkeep, conv=False):
    ...
    # Y_dp = tf.nn.dropout(Y_bn, keep_prob=pkeep)
    Y_dp = tf.nn.dropout(Y_fc, keep_prob=pkeep)
    Y_lr = leaky_relu(Y_dp, rate=0.01)
    return(Y_lr)
...
def conv_pipeline(X_in, in_width, out_width, fltr_conv, stride_conv, is_train, iteration, pkeep,
    ...
    Y_conv = tf.add(tf.nn.conv2d(X_in, W, strides=[1, stride_conv, stride_conv, 1], padding='SAME'), B)
